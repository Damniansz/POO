...
Connection con=cc.conexion();
  ...
  public Form_estu() {
    ...
    TablaEstudiante.addMouseListener(new java.awt.event.MouseAdapter() {
        ...
        public void mouseClicked(java.awt.event.MouseEvent evt) {
            ...
            if (fila >= 0) {
                TablaEstudiante.setRowSelectionInterval(fila, fila);
            }
        }
    });
  }
  ...
  @SuppressWarnings("unchecked")
    private String codigoEstudianteSeleccionado;
    ...
    public void mostrarDatos(){
        ...
        public void mouseClicked(java.awt.event.MouseEvent evt) {
            ...
            if (fila >= 0) {
                TablaEstudiante.setRowSelectionInterval(fila, fila);
                codigoEstudianteSeleccionado = TablaEstudiante.getValueAt(fila, 0).toString();
            }
        }
    });
   ...
   String SQL = "INSERT INTO estudiantes(estu_asignatura, estu_nombre, estu_apellido, estu_promedio, estu_estado)values(?,?,?,?,?)";
    ...
    if (codigoEstudianteSeleccionado == null) {
        JOptionPane.showMessageDialog(null, "Seleccione un estudiante para editar.");
        return;
    }

    String SQL = "SELECT * FROM estudiantes WHERE estu_codigo = ?";
    try {
        PreparedStatement pst = (PreparedStatement) con.prepareStatement(SQL);
        pst.setString(1, codigoEstudianteSeleccionado);
        ResultSet rs = pst.executeQuery();

        if (rs.next()) {
            // Cargar los datos del estudiante seleccionado en los campos de texto
            btnAsig.setSelectedItem(rs.getString("estu_asignatura"));
            btnNombre.setText(rs.getString("estu_nombre"));
            btnApellido.setText(rs.getString("estu_apellido"));
            btnProm.setText(rs.getString("estu_promedio"));
            btnEstado.setSelectedItem(rs.getString("estu_estado"));
        }
    } catch (Exception e) {
    ...
    private void btnRefrescarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRefrescarActionPerformed
        ...
        String codigoEstudiante = TablaEstudiante.getValueAt(filaSeleccionada, 0).toString();
        String nuevoAsignatura = btnAsig.getSelectedItem().toString();
        String nuevoNombre = btnNombre.getText();
        String nuevoApellido = btnApellido.getText();
        String nuevoPromedio = btnProm.getText();
        String nuevoEstado = btnEstado.getSelectedItem().toString();
        


        String SQL = "UPDATE estudiantes SET estu_asignatura=?, estu_nombre=?, estu_apellido=?, estu_estado=?, estu_promedio=?  WHERE estu_codigo=?";
        try {
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(SQL);
            
            pst.setString(1, nuevoAsignatura);
            pst.setString(2, nuevoNombre);
            pst.setString(3, nuevoApellido);
            pst.setString(4, nuevoEstado);
            pst.setString(5, nuevoPromedio);
